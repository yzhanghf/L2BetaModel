...
beta_est_df  = pd.read_csv('./data/AWS-csv/AWS_beta_est.txt',header=None)
...
while line:
    ...
    if x[3] not in concept_dict:
        continue
    else:
        concept_dict[x[3]]  += beta_est_dict[x[2]]*score
        concept_dict2[x[3]] += 1
...
concept_table.to_csv('data/AWS-csv/AWS_concept_rank.csv')
...
while line:
    ...
    if x[3] not in concept_dict:
        ...
        paper_score[paper_dict[x[2]]] += concept_dict[x[3]]
    
    count += 1

fp.close()

paper_predict_table = pd.DataFrame(paper_score, columns=['paper:score'])
paper_predict_table.to_csv('data/AWS-csv/AWS_paper_score.csv')
