...
while line:
    ...
    if relation_dict[x[1]]==2:
        ...
        # temp2 = paper_author_dict[x[0]]
        # temp2.append(x[2])
        # paper_author_dict[x[0]] = temp2
    
    if relation_dict[x[1]]==5 and 'covid19#>' not in x[0]:
        author_institution_dict[x[0]].append(x[2])
        # temp3 = author_institution_dict[x[0]]
        # temp3.append(x[2])
        # author_institution_dict[x[0]] = temp3
...
# Country code: Other=0; China=1; USA=2; UK=3; EU=4; JpKr=5; India=6
for iii in range(N):
                ...
                for institution_name in institution_vec:
                    if 'covid19#>' not in institution_name:
                        ...
                        if len(country_vec)>0:
                            # for country_name in country_vec:
...
# np.savetxt('./data/kg_Table.txt', paper_nation_tab, delimiter=",")
...
line = 456;count = 0
fp = open('data/metadata.csv', 'r', encoding='utf-8')
next(fp);
while line:
